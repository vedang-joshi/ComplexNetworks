'''
"Assuming nodes have been initialized by different types (opinions) in the voter model, the convergence of time of the
voter model (to a consensus) has the same distribution as the convergence time of the coalescing random walk process
(to a single particle)." taken from Yildiz, M.E. et al. “Voting models in random networks.” Information Theory and
Applications Workshop (ITA), 2010. 2010. 1-7. ©2010 Institute of Electrical and Electronics Engineers.
Demonstrated by T. M. Liggett, Interacting Particle Systems. Springer Berlin Heidelberg, 2005. and
                D. Aldous, “Random walks on finite groups and rapidly mixing markov chains,” in Seminaire
                de Probabilities XVII. Springer, 1983.

We code the upper bound of convergence times from the lemmas from Aldous and Lovasz's work along with the original work
by Yildiz et al for random graphs.

We run 10 simulations on every random graph generated with n nodes. We print
'''
...
def mainfunct(num_simulations):
    ...
    G = nx.gnm_random_graph(n=node_num, m=edge_num, directed=False)
    # Remove selfloops
    ...
    # Use largest connected component from now on as this is still a randomly generated graph

    # Set of nodes
    V = list(largest_subgraph.nodes)
    # Set of edges
    E = list(largest_subgraph.edges)
    ...
    L = nx.laplacian_matrix(largest_subgraph).toarray()

    # Adjacency matrix
    A = nx.adjacency_matrix(largest_subgraph)
    A = A.todense()
    ...
    expectedConvergenceTimes = ((4 * math.e * np.log(N + 2) * len(E))/(1 - lambda_2 * M) * np.max(inverseDiagonalD))
    ...
    for node_picked,node_conv in zip(randomNodePicked, actualConvergentNodeList):
        randomWalkConvList.append(expectedConvergenceTimes[np.array(node_picked), np.array(node_conv)])
...
expectedConvergenceTimesListOuter, randomWalkConvergenceList, num_nodes = mainfunct(num_simulations=400)
...
#plt.scatter(nodeOfConvergenceListOuter, list(map(operator.sub, randomWalkConvergenceList, iterateConvergenceListOuter)))
plt.xlabel('Final convergence of all opinions at node')
plt.ylabel('Number of iterations')
plt.title('RANDOM GRAPH: Number of simulations = 500, Number of nodes considered = %d'%num_nodes)
