...
def mainfunct(num_simulations):
    ...
    node_num = random.randint(2, 10)

    nodeList.append(node_num)
    # Generate graph
    G = nx.complete_graph(n=node_num)
    # Remove selfloops
    ...
    expectedConvergenceTimes = ((4 * math.e * np.log(N + 2) * N * (N-1) * 0.5)/(N/(N-1)) * (np.max(inverseDiagonalD)))
...
expectedConvergenceTimesListOuter, randomWalkConvergenceList, num_nodes = mainfunct(num_simulations=50)
...
plt.title('COMPLETE GRAPH: Number of simulations: 500, Number of nodes considered: %d'%num_nodes)
